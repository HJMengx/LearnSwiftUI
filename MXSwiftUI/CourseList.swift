...
struct CourseList: View {
    ...
    var body: some View {
        ZStack {
            ScrollView(showsIndicators: show ? false : true) {
                VStack(spacing: 30) {
                    Text("Courses")
                    ...
                    ForEach(courses.indices, id: \.self) { index in
                        GeometryReader { geometry in
                            CourseView(show: self.$courses[index].show, course: self.courses[index], active: self.$active, index: index, activeIndex: self.$activeIndex)
                                ...
                                .offset(x: self.activeIndex != -1 && self.activeIndex
                                    != index  ? screen.width : 0)
                                .scaleEffect(self.activeIndex != -1 && self.activeIndex
                                    != index  ? 0.5 : 1)
                        }
                        .frame(height: 280)
...
struct CourseView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            // 展示详细信息, 页面跳转
            if self.show {
                CourseDetial(course: course, show: $show, active: $active, activeIndex: $activeIndex)
...
// 适应暗黑模式, 字体大小模式, 可以将预先设置的颜色放入Assets, 分别设置好 Light/Dark 颜色值, 在代码中引用
// 字体使用 Title, Subtitle类似, 会根据设置自动调节, 特别是小字(fontSize < 20)
var courseData = [
